...
add_constraints([]) :-
	rule.
add_constraints([Constraint | Constraints ]) :-
	Constraint =.. [F | Args],
	...
	(
		F == '{}'
	->	transform_constraint(Args, New_Args),
...
head_arg(Arg, New_Arg) :-
	...
	(
		N < 2
	->	format(user_error, "ERROR: not enough arguments to ':'/2 in `~w`~n", [Arg]), fail
	;	true
	),
	(
		N > 2
	->	format(user_error, "ERROR: too many arguments to ':'/2 in `~w`~n", [Arg]), fail
	;	true
	),
	Dot_Args = [Object, Attribute],
	(
		var(Attribute)
	->	format(user_error, "ERROR: attempting to access variable attribute in `~w`~n", [Arg]), fail
	;	true
	),
	
	Attribute =.. [Attribute_F | Attribute_Args],
	(	
		Attribute_Args = []
	->	(
			atom(Attribute_F)
		->	find_fact3(Object1, Attribute_F, Value1, [Object1:Object], Subs1),
			get_sub(Value1, Subs1, New_Arg),
		;	format(user_error, "ERROR: attribute must be an atom in `~w`~n", [Arg]), fail
		)
	;	(
			Attribute_F = ':',
			Attribute_Args = [X, Y | Rest_Attribute]
		->	(
				atom(X)
			->	find_fact3(Object1, X, Value1, [Object1:Object], Subs1),
				get_sub(Value1, Subs, Next_Object)
				Continuation =.. [':' | [Next_Object, 
			;	format(user_error, "ERROR: attribute must be an atom in `~w`~n", [Arg]), fail
			)
		;	format(user_error, "ERROR: invalid attribute continuation `~w` in `~w`~n", [Attribute, Arg]), fail
		)
	).
*/
head_arg(Arg, New_Arg) :-
