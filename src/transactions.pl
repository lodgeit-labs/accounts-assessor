...
% Relates Date to the balance of the given account at that time.
...
test00 :- 
    ...
    % ----------------------------------------------------------------------------------------
    % Result should be
    X = trial_balance([ (bank, t_term(40, 0)), (inventory, t_term(0, 0)),
			...
			(wages_payable, t_term(0, 0)), (hirepurchase_truck, t_term(0, 0))],
		      ...
		      [ (sales, t_term(0, 0))], [ (cost_of_goods_sold, t_term(0, 0)), (stationary, t_term(10, 0)),
						  (wages, t_term(0, 0)), (super_expense, t_term(0, 0)),
...
test10 :-
    ...
    % ----------------------------------------------------------------------------------------
    % Result should be
    X = balance_sheet([ (bank, t_term(40, 0)), (inventory, t_term(0, 0)),
			(accounts_receivable, t_term(100, 0)), (motor_vehicles, t_term(0, 0))],
		      [ (accounts_payable, t_term(0, 0)), (super_payable, t_term(0, 0)), (paygw_tax, t_term(0, 0)),
			(wages_payable, t_term(0, 0)), (hirepurchase_truck, t_term(0, 0))],
...
test15 :-
    ...
    % ----------------------------------------------------------------------------------------
    % Result should be
    X = movement([ (bank, t_term(0, 299)), (inventory, t_term(75, 0)),
		 ...
		 [ (share_capital, t_term(0, 0))], [ (sales, t_term(0, 100))],
		 [ (cost_of_goods_sold, t_term(50, 0)), (stationary, t_term(0, 0)),
		   (wages, t_term(200, 0)), (super_expense, t_term(19, 0)),
...
% Result should be
    Retained_Earnings = t_term(50, 100), Retained_Earnings_Signed = 50.
...
% Let's get the retained earnings as of date(19, 6, 2):
    ...
    credit_isomorphism(Retained_Earnings, Retained_Earnings_Signed),
% Result should be
    Retained_Earnings = t_term(60, 100), Retained_Earnings_Signed = 40.
...
% Let's get the current earnings between date(17, 7, 1) and date(17, 7, 3):
    ...
    % Result should be
    Current_Earnings = t_term(50, 100), Current_Earnings_Signed = 50.
...
% Let's get the current earnings between date(18, 7, 1) and date(19, 6, 2):
    ...
    credit_isomorphism(Current_Earnings, Current_Earnings_Signed),
    % Result should be
    Current_Earnings = t_term(10, 0), Current_Earnings_Signed = -10.
...
% What is the isomorphism of the inventory account?
    ...
    % Result should be
    Account_Type = asset, Isomorphism = debit_isomorphism.
...
% Let's get the net activity of the asset-typed account between date(17, 7, 2) and date(17, 7, 3).
    ...
    % Result should be
    Net_Activity = t_term(150, 50).




:- test00, test10, test15, test20, test30, test40, test50, test60, test70, test80, test90.
