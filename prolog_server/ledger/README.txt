...
# the endpoint
The only way to feed data into it is through a ledger request xml, which contains bank statements.
We generate the request xml's from an excel spreadsheet file, using some templates and a (.NET?) plugin.

A bank statement is a series of records of what amounts went into or from your bank account. Each bank account has a single currency. Users are able to annotate each record with a transaction type.
Transaction types, aka action taxonomy, are defined in one sheet of the excel file. A transaction type has a name, description, an exchanged account, and an optional trading account. 
The other input in the xml file can be livestockData, containing number of births, deaths, etc.
...
'with_cost_per_unit': if a lookup into the exchange rates table fails, the value of the unit is taken from this term.
...
Each s_transaction can result in multiple transactions. 

First, we record the change on the bank account. This transaction is always in the currency of the bank account.

An "exchanged" account, or "counteraccount", is specified in the transaction type. If the bank statement record is not annotated with an exchanged unit type, the transaction is probably a revenue or expense. We record it in the account specified in the transaction type, but with the value converted into report currency, using an exchange rate for that date. 
