...
clean_terminal :-
	%shell2('echo -e "\e[3J"'), % for some strange reason, running reset from here actually does clear the scrollback in konsole, rendering this echo unnecessary
	shell2('reset').

x :-
	((current_prolog_flag(argv, [Viewer, Script, Goal]),!)
		;(format(user_error, 'argument parsing failed.', []), halt)),
	atomic_list_concat(['unbuffer swipl -s ', Script], Load_Cmd),
	clean_terminal,
